...
namespace ImGuiEx::BigTable {
    ...
    {
        //ImGuiContext& g = *GImGui;
        TempBuffer.reserve((int)(sizeof(ImGuiTableFixDisplayOrderColumnData) * table->ColumnsCount)); // FIXME: Maybe wrap those two lines as a helper.
        ImGuiTableFixDisplayOrderColumnData* fdo_columns = (ImGuiTableFixDisplayOrderColumnData*)(void*)TempBuffer.Data;
    ...
    bool BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, ImGuiWindowFlags subWindowFlags, const ImVec2& outer_size, float inner_width)
    {
        ...
        if (++TablesTempDataStacked > TablesTempData.Size)
            TablesTempData.resize(TablesTempDataStacked, ImGuiTableTempData());
        ImGuiTableTempData* temp_data = table->TempData = &TablesTempData[TablesTempDataStacked - 1];
        ...
        if (use_child_window)
        {
            ...
            if ((previous_flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) == 0)
                if ((NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasScroll) == 0)
                    SetNextWindowScroll(ImVec2(0.0f, 0.0f));

            // Create scrolling region (without border and zero window padding)
            ImGuiChildFlags child_child_flags = (NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasChildFlags) ? NextWindowData.ChildFlags : ImGuiChildFlags_None;
            ImGuiWindowFlags child_window_flags = (NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasWindowFlags) ? NextWindowData.WindowFlags : ImGuiWindowFlags_None;
            child_window_flags |= subWindowFlags;
            if (flags & ImGuiTableFlags_ScrollX)
    ...
    void EndTable()
    {
        ...
        */
        ...
        if (inner_window != outer_window)
        {
            ...
            CurrentTable = NULL; // To avoid error recovery recursing
            EndChild();
            CurrentTable = table;
        ...
        IM_ASSERT(TablesTempDataStacked > 0);
        temp_data = (--TablesTempDataStacked > 0) ? &TablesTempData[TablesTempDataStacked - 1] : NULL;
        CurrentTable = temp_data && (temp_data->WindowID == outer_window->ID) ? Tables.GetByIndex(temp_data->TableIndex) : NULL;
        if (CurrentTable)
        {
            CurrentTable->TempData = temp_data;
            CurrentTable->DrawSplitter = &temp_data->DrawSplitter;
        }
    ...
    ImVec2 GetContentRegionAvail()
    {
        ImGuiContext& g = *GImGui;
        ImGuiWindow* window = g.CurrentWindow;
        ImVec2 mx = (window->DC.CurrentColumns || CurrentTable) ? window->WorkRect.Max : window->ContentRegionRect.Max;
    ...
    static void TableSettingsHandler_ClearAll(ImGuiContext* ctx, ImGuiSettingsHandler*)
    {
        ...
        for (int i = 0; i != Tables.GetMapSize(); i++)
            if (ImGuiTable* table = Tables.TryGetMapData(i))
                table->SettingsOffset = -1;
    ...
    void MenuItemTableColumnVisibility(ImGuiTable* table, int columnIdx)
    {
        ...
        if (ImGui::MenuItem(columnName, NULL, column.IsUserEnabled, menu_item_active))
